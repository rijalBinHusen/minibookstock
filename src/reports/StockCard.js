...
export const stockCard = async (item_id, tanggal_start, tanggal_finished) => {
  ...
  if (stockMasters.length) {
    ...
    for (const [index, stock] of stockMasters.entries()) {
      ...
      const dateStartTime = new Date(tanggal_start).getTime();
      const dateEndTime = new Date(tanggal_finished).getTime();

      const isPassingCondition =
        stock?.available_start <= dateEndTime &&
        stock?.available_end >= dateStartTime;
    ...
    const filterTempResult = tempRes.filter(
      (res) =>
        res.unix_time >= tanggal_start && res.unix_time <= tanggal_finished
    );
    // sort result
    ...
    if (sortFilterResult.length) {
      ...
      alert("Tidak ada transaksi untuk periode tersebut!");
    }
  } else {
    alert("Belum terdapat transaksi!");
  }
};
