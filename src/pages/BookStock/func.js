...
import { getTotalStockTaken, store as storeOutput } from "../../composables/Output"
...
class Stock {
    constructor (
        id, available, available_start, available_end, created, incoming_parent_id, 
        isTaken, item_id, kd_produksi, product_created, quantity,
        incomeShift1, incomeShift2, incomeShift3, incomeShift4, outputShift1, outputShift2, outputShift3, outputShift4
    ) {
        this.id = id
    ...
    setStockAwal() {
        this.stockAwalShift1 = this.quantity + this.outputShift1 + this.outputShift2 + this.outputShift3 + this.outputShift4 - this.incomeShift1 - this.incomeShift2 - this.incomeShift3 - this.incomeShift4
        this.stockAwalShift2 = this.quantity + this.outputShift2 + this.outputShift3 + this.outputShift4 - this.incomeShift2 - this.incomeShift3 - this.incomeShift4
        this.stockAwalShift3 = this.quantity + this.outputShift3 + this.outputShift4 - this.incomeShift3 - this.incomeShift4
    }
}

export async function getBookStock (date) {
    const dateTime = new Date(date).getTime()
    // function to get stock master >= date to show && <= date to show
    const getStocks = await getStockForBookStock(dateTime)

    const stocks = getStocks.map((rec) => new Stock(rec?.id, rec?.available, rec?.available_start, rec?.available_end, rec?.created, rec?.icoming_parent_id, rec?.isTaken, rec?.item_id, rec?.kd_produksi, rec?.product_created, rec?.quantity, 0, 0, 0, 0, 0, 0, 0, 0))
    ...
    for (let incomeLevel1 of incomes) {
        for(let stockMasterId of incomeLevel1.stock_master_ids) {
            ...
            if(findRec) {
                ...
                findRec.addIncome(incomeLevel1?.shift, findRec?.quantity + stockTaken.allTaken )
            }
        }
    }

    const outputDB = useIdb(storeOutput)
    const outputMoreThanDate = await outputDB.getItemsByKeyGreaterThan('tanggal', dateTime)
    const outputDate = await outputDB.getItemsByKeyValue('tanggal', ymdTime(dateTime))

    // // sum quantity
    outputMoreThanDate.forEach((rec) => {
        const findRec = stocks.find((recLv2) => recLv2?.id === rec?.stock_master_id)
        if(findRec) {
    ...
    outputDate.forEach((rec) => {
        const findRec = stocks.find((recLvl2) => recLvl2?.id === rec?.stock_master_id)
        if(findRec) {
    ...
    stocks.forEach((rec) => {
        const findRec = finalStock.find((recLvl2) => recLvl2?.item_id === rec?.item_id)
        if(findRec) {
            findRec?.addQuantity(rec?.quantity)
