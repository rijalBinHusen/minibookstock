...
describe('Click submit button ', async () => {
  ...
  // new record
  const kd_item = faker.datatype.string(10);
  const nm_item = faker.datatype.string(50);
  ...
  it('Value in form must be equal', () => {
  ...
  const table_age_ite = wrapper.find('#table-master-item-row-0-column-2');
  ...
  it('Value in table must equal to new record variable', async () => {
    ...
    expect(table_age_ite.text()).equal(age_item);
