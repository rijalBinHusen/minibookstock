...
for(let i = 2; i <= lengthRow; i++) {
    ...
    if(sheet["A"+i]) {
        await createStockAwal(kode_item, nama_item, umur_product, quantity, tanggal_produksi)
    }
}
  // close the loader
...
const handleFormat = () => {
    // format
    const form = [
        { kode_item: 'KODEITEM', nama_item: 'NAMA ITEM', quantity: 100, 'umur_produk (Bulan)': 7, tanggal_produksi: '2022/03/23' }
    ]
