const getDetails = (payload) => {
  ...
  const minutesNumber =
    jsDate.getMinutes() > 9 ? jsDate.getMinutes() : "0" + jsDate.getMinutes();
  return {
    origin: jsDate,
    date: dateNumber,
    month: monthNumber,
...
export const ymdTime = (payload) => {
  ...
  return new Date(
    details.year + "/" + (details.month + 1) + "/" + details.date
  ).getTime();
};
...
export const ddmmyyyy = (payload, splitter) => {
  ...
  return (
    details.date +
    splitter +
    (details.month > 8 ? details.month + 1 : "0" + (details.month + 1)) +
    splitter +
    details.year
  );
};

export const dayPlusOrMinus = (yourDate, yourNumberOfDay) => {
  let date = yourDate ? new Date(yourDate) : new Date();
  date.setDate(date.getDate() + yourNumberOfDay);
  return date.getTime();
};
